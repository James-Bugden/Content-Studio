...
- Backlog posts show a bounded 40-row inventory with source, platform, PESTO, hook template, alternatives, draft excerpt and a status. Filters include source, platform and computed status; sort can use source, hook or status. Search runs as an authenticated same-origin request with the term in the body, never in the URL or telemetry. The table defaults to compact rows and keeps its hook columns visible, with optional controls to change both; phones use cards with the same key facts.
- Backlog status is derived, not written: a queued Library row can be called **Ready to schedule** only when its derived Ready Queue entry matches the Library and the existing release gates pass. **Ready for Typefully** additionally requires a matching Schedule lineage, the same hook and draft, and the Schedule next step to say Send to Typefully. Missing provider data is unknown; mismatched views require reconciliation. Review labels such as Drafting, Approved, Needs changes and Rejected do not imply readiness.
- The Backlog panel shows the guarded draft editor first; English check and hook review can be opened when needed. Previous and Next move through the visible posts and retain the dirty-edit confirmation. Server telemetry records only durations for backlog.load, backlog.search and editor.load, with per-instance p50/p95 in Fix issues. Short 15-second caches for Library, Ready Queue and Schedule speed repeated browsing; every mutation compares against fresh provider state and clears local cached reads on success. Primary navigation does not prefetch the other data-heavy routes, to avoid spending Sheet quota before a user opens them. A rate-limited Library read shows a retry state rather than a generic server error.
