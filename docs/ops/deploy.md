...
| `SUPABASE_READ_MODEL_URL`, `SUPABASE_READ_MODEL_SERVICE_KEY` | unset | approved shared product project | Server-only. Content Studio may access only its `content_studio_*` objects. |
...
| `CRON_SECRET` | unset | unset until reconciliation is enabled | Random 16+ character bearer secret generated by the owner. Vercel sends it only to the registered cron route. |
...
5. **Optional Supabase read model:** use the approved shared future combined-product project, apply only the committed Content Studio migration, and configure the four `SUPABASE_READ_MODEL_*` variables against a copied Sheet first. Never run a remote reset, whole-project schema pull or repo-wide migration push from Content Studio. Keep mode `off` in production until MIG-02 through MIG-05 evidence is accepted.
...
1. Apply `supabase/migrations/20260925030000_sheet_read_model.sql` explicitly to the approved project. Do not use a repo-wide `db push` because the project has another app's migration history.
...
For later continuous reconciliation, the fail-closed hook is `GET /api/ops/sheet-mirror/reconcile`. Vercel Cron sends `Authorization: Bearer <CRON_SECRET>`; do not register a schedule until the approved Supabase project, copied Sheet, secret, advisor pass and first manual snapshot have all been verified. Then add this production-only project configuration and redeploy:

```json
{
  "crons": [{ "path": "/api/ops/sheet-mirror/reconcile", "schedule": "*/15 * * * *" }]
}
```

Confirm one scheduled run before relying on it. Cron timing is not guaranteed, so alert on stale `completed_at` rather than exact invocation time. Overlapping snapshot finalizers are serialized per source and an older run is refused, so duplicate scheduler delivery cannot regress the active mirror. Removing the `crons` entry and redeploying disables future invocations; setting mirror mode to `off` blocks reconciliation without changing Sheet behavior.

### Direct API denial proof (MIG-04)

After applying the migration to the approved project, use that project's public anon key only:

```sh
SUPABASE_READ_MODEL_URL=https://PROJECT.supabase.co \
SUPABASE_READ_MODEL_TEST_ANON_KEY=PUBLIC_ANON_KEY \
npm run verify:supabase
```

Both the active-row table read and the deliberately invalid/non-mutating snapshot RPC probe must report `denied`. The script never uses or prints the service-role key or response bodies. Then run Supabase's database and security advisors and attach their clean output, the exact migration SHA, and the copied-Sheet parity result to issue #21. Keep MIG-04 deferred until those live checks exist.
