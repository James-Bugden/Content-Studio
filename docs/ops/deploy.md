...
For later continuous reconciliation, the fail-closed hook is `GET /api/ops/sheet-mirror/reconcile`. Vercel Cron sends `Authorization: Bearer <CRON_SECRET>`; do not register a schedule until the dedicated Supabase project, copied Sheet, secret, advisor pass and first manual snapshot have all been verified. Then add this production-only project configuration and redeploy:
...
After applying the migration to the dedicated non-production project, use that project's public anon key only:
